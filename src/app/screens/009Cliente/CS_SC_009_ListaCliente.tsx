...
import {
  Csicp_bb012,
  IResGetListConta,
} from "../../services/api/interfaces/contas/CS_IResGetListConta";
...
import {
  handleAnaliseCliente,
  handleGerarCliente,
} from "../../view_controller/crediario/CrediarioViewController";
...
const CS_SC_009_ListaCliente = ({ route }: { route: any }) => {
  ...
  const [currentClientSelected, setCurrentClientSelected] =
    useState<Csicp_bb012>();
  ...
  function getClientesList(page: number, searchValue: string) {
    ...
    handleGetListConta({
      ...
      cs_codigoConta: /^\d+$/.test(searchValue) && searchValue.length != 11 && searchValue.length != 14 ? Number.parseInt(searchValue) : undefined,
      cs_cpfcnpj: /^\d+$/.test(searchValue) && searchValue.length === 11 || searchValue.length === 14 ? searchValue : undefined
    })
      .then((res) => {
        try {
          if (res !== undefined) {
            setClientList(res);
            const pagesArray = getPaginationList(
              res.PageSize.cs_number_of_pages
            );
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      {!isLoading ? (
        <>
          <FlatList
            ListEmptyComponent={
              <CustomEmpty text="Nenhum cliente encontrado!" />
            }
            ...
            renderItem={({ item }) => (
              <RenderItemCliente
                handlePopUp={() => handlePopUp(item)}
                cliente={item}
                edit={(bb12id) => handleClickItem(bb12id)}
              />
            )}
          />
        </>
        ...
        <ActivityIndicator
          style={[commonStyle.align_centralizar, { height: "100%" }]}
          size="large"
          color={ColorStyle.colorPrimary200}
        />
      )}
      ...
      <CustomAlertDialog
        ...
        children={
          <AlertDialog
            cliente={currentClientSelected!}
            onClose={(cliente) => handlePopUp(cliente)}
          />
        }
      />
    </SafeAreaView>
...
const RenderItemCliente = ({
  cliente,
  edit,
  handlePopUp,
}: {
  cliente: Csicp_bb012;
  handlePopUp: (cliente: Csicp_bb012) => void;
  edit: (id: string) => void;
}) => {
  return (
    <CustomCard004
      ...
      children={
        <View style={commonStyle.common_columnItem}>
          <Text style={commonStyle.common_fontWeight_800}>
            {cliente.csicp_bb012.csicp_bb012.BB012_Codigo}
          </Text>
          <Text style={commonStyle.common_fontWeight_800}>
            {cliente.csicp_bb012.csicp_bb012.BB012_Nome_Cliente}
          </Text>
          <Text style={commonStyle.common_fontWeight_800}>
            {cliente.BB01202.csicp_bb01202.BB012_CPF ||
              cliente.BB01202.csicp_bb01202.BB012_CNPJ}
          </Text>
          <Text style={[commonStyle.common_fontWeight_800]}>
            {cliente.csicp_bb012.csicp_bb012_SitCta.Label}
          </Text>
          <Text style={[commonStyle.common_fontWeight_800]}>
            Limite de crédito:{" "}
            {moneyApplyMask(cliente.BB01201.csicp_bb01201.BB012_LimiteCredito)}
          </Text>
        </View>
      }
      title={cliente.csicp_bb012.csicp_bb012.BB012_Nome_Cliente}
      rightItem={
        <RightItemCliente cliente={cliente} handlePopUp={handlePopUp} />
      }
    />
  );
};

const RightItemCliente = ({
  cliente,
  handlePopUp,
}: {
  cliente: Csicp_bb012;
  ...
  return (
    ...
    >
      <CustomIcon
        icon={ICON_NAME.TRES_PONTOS_VERTICAL}
        onPress={() => {
          handlePopUp(cliente);
        }}
      />
...
const AlertDialog = ({
  ...
  cliente: Csicp_bb012;
  onClose: (cliente: Csicp_bb012) => void;
}) => {
  const [isBtnLoading, setIsBtnLoading] = useState(false);
  const { navigate } = useNavigation();
  const [resultOfAnalysis, setResultOfAnalysis] =
    useState<IResAnaliseCliente>();
  ...
  return (
    <View style={stylesEntregaCard.dialog}>
      ...
      {isBtnLoading && <ActivityIndicator />}

      {!isBtnLoading && (
        <View
          style={[
            stylesEntregaCard.contentContanier,
            commonStyle.common_columnItem,
          ]}
        >
          <View style={[{ width: "95%" }]}>
            <TouchableOpacity
              onPress={() => {
                ...
                handleGerarCliente({
                  cs_conta_id: cliente.csicp_bb012.csicp_bb012.ID,
                })
                  .then(() => {
            ...
            >
              <View style={[commonStyle.common_rowItem, commonStyle.margin_16]}>
                ...
                >
                  <CustomIcon
                    icon={ICON_NAME.ADICIONAR_PESSOA_CONTORNADO}
                    iconColor="#FFF"
                  />
                </View>
                <View style={[commonStyle.align_centralizar]}>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Cadastre no meu crediário clicando aqui!
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              onPress={() => {
                /** ANALISE DE CREDITO */
                navigate("ConsultaScore", {
                  bb12id: cliente.csicp_bb012.csicp_bb012.ID,
                });
              }}
            >
              <View style={[commonStyle.common_rowItem, commonStyle.margin_16]}>
                ...
                >
                  <CustomIcon icon={ICON_NAME.CASH} iconColor="#FFF" />
                </View>
                <View style={[commonStyle.align_centralizar]}>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Efetue análise de crédito clicando aqui!
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
            ...
            >
              <View style={[commonStyle.common_rowItem, commonStyle.margin_16]}>
                ...
                >
                  <CustomIcon
                    icon={ICON_NAME.ESTATISTICA_CONTORNADO}
                    iconColor="#FFF"
                  />
                </View>
                <View style={[commonStyle.align_centralizar]}>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Efetue uma simulação de venda do cliente
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
            ...
            {resultOfCreatingUser !== "" && resultOfAnalysis === undefined && (
              <View style={[commonStyle.align_centralizar]}>
                <Text
                  style={[
                    commonStyle.common_text_button_style,
                    { color: "green" },
                  ]}
                >
                  {resultOfCreatingUser}
                </Text>
              </View>
            )}
            ...
            {resultOfCreatingUser === "" && resultOfAnalysis !== undefined && (
              ...
              >
                <View style={commonStyle.common_columnItem}>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Limite de Crédito:{" "}
                    {formatMoneyValue(resultOfAnalysis.limite.saldo)}
                  </Text>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Tipo de Cliente: {resultOfAnalysis.limite.tipocliente}
                  </Text>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Risco: {resultOfAnalysis.limite.risco}
                  </Text>
                  <Text style={[commonStyle.common_text_button_style]}>
                    Ação Sugerida: {resultOfAnalysis.limite.acaoSugerida}
                  </Text>
                </View>
                <CustomIcon
                  icon={ICON_NAME.FECHAR}

...
import { IResComandaList } from "../../interfaces/comanda/CS_IResListaComanda";
...
export async function getListaComanda({ cs_tenant_id }: { cs_tenant_id: number }): Promise<IResComandaList> {
    try {
        ...
        const response = await api.get('/CSR_DD100_PreVenda/rest/CS_Comandas/comanda_get_list', { headers: data });
...
export async function insereProdutoComanda({ dataInsertComanda, cs_tenant_id }: { dataInsertComanda: IComandaDataInsert, cs_tenant_id: number }) {
    try {
        ...
        const response = await api.post('/CSR_DD100_PreVenda/rest/CS_Comandas/comanda_insere_produto', dataInsertComanda, { headers: data });
...
export async function removerProdutoComanda({ cs_tenant_id, comanda_produto_id }: { cs_tenant_id: number, comanda_produto_id: number }) {
    try {
        ...
        const response = await api.put('/CSR_DD100_PreVenda/rest/CS_Comandas/comanda_update_qtd_produto', { headers: data });
...
export async function getComandaById({ cs_tenant_id, comanda_id }: { cs_tenant_id: number, comanda_id: number }): Promise<CS_IResComandaById> {
    try {
        ...
        const response = await api.get('/CSR_DD100_PreVenda/rest/CS_Comandas/comanda_get', { headers: header });

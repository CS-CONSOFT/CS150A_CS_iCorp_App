...
export async function handleGetContaById({ cs_conta_id }: { cs_conta_id: any }) {
    try {
        ...
        const response = await getContaById({ cs_conta_id: cs_conta_id, cs_tenant_id: currentUser.TenantId });
        return response;
    } catch (error) {
        throw error;
    }

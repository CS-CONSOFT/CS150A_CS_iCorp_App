...
export async function handleRI_Gerar_OBS_RI({ In_GG071_ID, In_Observacao }:
    { In_GG071_ID: number, In_Observacao: string }) {
    try {
        // Obtém o usuário atual do armazenamento
        const currentUser = await getObject(DataKey.LoginResponse) as ILoginResponse;

        // Faz uma requisição para salvar os dados de endereço
        const response = await RI_Gerar_OBS_RI({ In_GG071_ID: In_GG071_ID, In_Observacao: In_Observacao, cs_tenant_id: currentUser.TenantId });
        return response;
    } catch (error) {
        throw error;
    }
